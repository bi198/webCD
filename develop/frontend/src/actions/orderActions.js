...
const createOrder = (order) => async (dispatch, getState) => {
  try {
    ...
    const { data: { data: newOrder } } = await Axios.post("/api/orders/create", order,userInfo);
...
const listMyOrders = () => async (dispatch, getState) => {
  try {
    ...
    const { userSignin: { userInfo } } = getState();
    //  const { data } = await axios.post("/api/products/createProduct2", product);
    const { data } = await Axios.get("/api/orders/mine",userInfo);
...
const listOrders = () => async (dispatch, getState) => {
  ...
  try {
    ...
    const { userSignin: { userInfo } } = getState();
     const { data } = await Axios.get("/api/orders",userInfo);
...
const detailsOrder = (orderId) => async (dispatch, getState) => {
  try {
    ...
    const { userSignin: { userInfo } } = getState();
    const { data } = await Axios.get("/api/orders/" + orderId,userInfo);
